...
from utils import *
import unet3d
from config import models_genesis_config, log
...
for i, fold in enumerate(tqdm(conf.train_fold)):
    ...
    s = np.load(os.path.join(conf.data, f'{conf.scale}X{conf.input_rows}X{conf.input_cols}X{conf.input_deps}', file_name))
    x_train.extend(s)
...
log.info("x_train: {} | {:.2f} ~ {:.2f}".format(x_train.shape, np.min(x_train), np.max(x_train)))
log.info("x_valid: {} | {:.2f} ~ {:.2f}".format(x_valid.shape, np.min(x_valid), np.max(x_valid)))
...
log.info("Total CUDA devices: ", torch.cuda.device_count())
...
if conf.weights != None:
    ...
    log.info("Loading weights from ", conf.weights)
sys.stdout.flush()
...
for epoch in range(intial_epoch, conf.nb_epoch):
    ...
    for iteration in range(int(x_train.shape[0] // conf.batch_size)):
        ...
        if (iteration + 1) % 5 == 0:
            log.info('Epoch [{}/{}], iteration {}, Loss: {:.6f}'
    ...
    log.info("Epoch {}, validation loss is {:.4f}, training loss is {:.4f}".format(epoch + 1, valid_loss, train_loss))
    train_losses = []
    valid_losses = []
    if valid_loss < best_loss:
        log.info("Validation loss decreases from {:.4f} to {:.4f}".format(best_loss, valid_loss))
        ...
        }, os.path.join(conf.model_path, "Genesis_OCT.pt"))
        log.info("Saving model ", os.path.join(conf.model_path, "Genesis_OCT.pt"))
    else:
        log.info("Validation loss does not decrease from {:.4f}, num_epoch_no_improvement {}".format(best_loss,
