...
data_save_dir = os.path.join(options.save, f'{options.scale}X{crop_rows}X{crop_cols}X{crop_depth}')
...
def infinite_generator_from_one_volume(config, img_array):
    ...
    slice_set = np.zeros((config.scale, crop_rows, crop_cols, crop_depth), dtype=float)
    ...
    while True:
        ...
        aug = get_augmentation_volume((crop_rows, crop_cols, crop_depth))
        data = {'image': img_array}
        aug_data = aug(**data)
        crop_window = aug_data['image']
        # crop_window = np.expand_dims(img_array, axis=0)
...
def get_self_learning_data(fold, config):
    ...
    for index_subset in fold:
        ...
        file_list = glob(os.path.join(luna_subset_path, "*.nii.gz"))
        ...
        for img_file in tqdm(file_list):
            ...
            img_oct_array = nibabel.load(img_file)
            img_array = img_oct_array.get_data()
            # img_array = img_oct_data.transpose(2, 1, 0)
